...
import PyMieScatt as mie # Longkang added for mie calculation
...
alt_read = np.loadtxt(vulcan_cfg.alt_path) # added by Longkang for given altitude

# Longkang added for tau of clouds (1), hazes (2) and UV absorber (UV)
N_p = np.loadtxt(vulcan_cfg.N_particle_path, skiprows=1)
UV_read = np.loadtxt(vulcan_cfg.UV_absorber_path, skiprows=1)
D_1 = 0.98E-4  # cm
D_2 = 2.4E-4
refractive_indx = 1.45 # refractive index
g_alt_Np1 = interpolate.interp1d(N_p[:, 0], N_p[:, 1], kind='slinear')
g_alt_Np2 = interpolate.interp1d(N_p[:, 0], N_p[:, 2], kind='slinear')
N_1 = np.array(g_alt_Np1(alt_read[1:] / 1E5)) # cm-3
N_2 = np.array(g_alt_Np2(alt_read[1:] / 1E5))
g_wavelen_Q = interpolate.interp1d(UV_read[:, 0], UV_read[:, 1], kind='slinear')
D_UV = D_1
N_UV = N_1
...
class ReadRate(object):
    ...
    def read_rate(self, var, atm):
        ...
        # Longkang added for modifying chemical network
        k = modify_chem.modify(k)
    ...
    def rev_rate(self, var, atm):
        ...
        for i in rev_list:
            ...
            if np.any(var.k[i] > 1.e-6):
                print ('R' + str(i) + " " + var.Rf[i-1] +' :  ' + str(np.amax(var.k[i])) )
                var.k[i] = np.ones(nz)*1E-20
            if np.any(var.k[i-1] > 1.e-6):
                print ('R' + str(i-1) + " " + var.Rf[i-1] + ' :  ' + str(np.amax(var.k[i-1])) )
                var.k[i-1] = np.ones(nz) * 1E-20
...
class Integration(object):
    ...
    def update_mu_dz(self, var, atm, make_atm): #y, ni, spec, Tco, pco
        ...
        atm.zco = alt_read  # added by Longkang for reading altitude from file
        for i in range(pref_indx,nz):
            ...
            atm.dz[i] = atm.zco[i + 1] - atm.zco[i]  # added by Longkang for given altitude
    ...
    def conv(self, var, para, atm, out=False, print_freq=100):
        ...
        longdy = np.abs((y_time[count-1] - y_time[indx])/np.vstack(atm.n_0)) # (dy between step count-1 and indx)/total density
        ...
        longdy = np.amax( longdy[ymix>0]/ymix[ymix>0] ) # the largest percentage of the ymix variation
        longdydt = longdy/(t_time[-1]-t_time[indx]) # the largest mean percentage of the varying ymix in uint time
...
class ODESolver(object):
    ...
    def diffdf_no_mol(self, y, atm):  # used by Longkang Dai in Venusian middle and lower amtospheres
        """
...
#         atm.Hpi = Hpi
    ...
    def lhs_jac_no_mol(self, var, atm): # used by Longkang Dai in Venusian middle and lower amtospheres
        """
        directly constructing lhs = 1./(r*h)*sparse.identity(ni*nz) - dfdy 
    ...
    def compute_tau(self, var, atm):
        ...
        if var.t == 0:
            var.tau_1 = np.zeros([nz,var.nbin])
            var.tau_2 = np.zeros([nz,var.nbin])
            var.tau_UV = np.zeros([nz,var.nbin])
        ...
        for j in range(nz-1,-1,-1):
            for sp in absp_sp:
                ...
                # Longkang Dai modified ===========================================================
                '''if j == nz-1:
                    if var.y[j,species.index(sp)] != 0 and var.y[j, species.index(sp)] != var.y[j-1, species.index(sp)]:
                        H_top = atm.dz[j]/np.log(var.y[j-1, species.index(sp)]/var.y[j, species.index(sp)])
                        H_top = abs(H_top)
                        if sp in vulcan_cfg.T_cross_sp:
                            var.tau[j] += var.y[j, species.index(sp)] * H_top * var.cross_T[sp][j]
                        else:
                            var.tau[j] += var.y[j, species.index(sp)] * H_top * var.cross[sp]
                else:
                    if sp in vulcan_cfg.T_cross_sp:
                        var.tau[j] += atm.dz[j] * 0.5 * (var.y[j, species.index(sp)] * var.cross_T[sp][j]+var.y[j+1, species.index(sp)] * var.cross_T[sp][j+1])
                    else:
                        var.tau[j] += atm.dz[j] * var.y[j, species.index(sp)] * var.cross[sp]'''
                # =================================================================================
...
#         # devided by the Eddington coefficient to recover the intensity
    ...
    def compute_J(self, var, atm): # the vectorized version
        ...
        # reset to zeros every time, sp = str(species), bn = int(branch+1), bn=0 means total
        ...
        for sp in var.photo_sp:
            ...
            # test
            if sp == 'CO2':
                np.savetxt('debug/CO2_J.txt',var.J_sp[(sp, 1)].T)
                np.savetxt('debug/Flux.txt',flux)
                np.savetxt('debug/bins.txt',bins)
                np.savetxt('debug/corss_CO2.txt',np.vstack((bins,diss_cross[(sp,1)])).T,fmt='%-11.5e')
...
class Ros2(ODESolver):
    ...
    def solver(self, var, atm, para):
        ...
        if vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == False:
            ...
            diffdf = self.diffdf_no_mol # used by Longkang Dai in Venusian middle and lower amtospheres
            jac_tot = self.lhs_jac_no_mol # used by Longkang Dai in Venusian middle and lower amtospheres
        ...
        # ==== fix sp from start by Longkang =====
        '''if vulcan_cfg.fix_sp_start:
            for sp in vulcan_cfg.fix_sp_start:
        ...
        if vulcan_cfg.use_print_delta == True and para.count % vulcan_cfg.print_prog_num==0:
            ...
            #aaa = delta/sol # Longkang added for debug
            #print('Max delta/sol=')
            #print ( [aaa[bbb, max_indx[bbb]] for bbb in range(len(aaa))] ) # Longkang added for debug
    ...
    def solver_fix_all_bot(self, var, atm, para):
        ...
        # Longkang added for stroring the diffusion and chemical terms
        var.diff_term = diffdf(y, atm)
        var.chem_term = chemdf(y,M,k)
        diff_term = var.diff_term.copy()
        chem_term = var.chem_term.copy()

        df = chem_term.flatten() + diff_term.flatten() # Longkang modified for stroring the diffusion and chemical terms
        #df = chemdf(y, M, k).flatten() + diffdf(y, atm).flatten()
        lhs = jac_tot(var, atm)
...
class Output(object):
    ...
    def save_out(self, var, atm, para, dname): 
        ...
        # ================ Longkang added for inserting diffusion and chemical terms ==================
        '''if vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == False:
            diffdf = ODESolver.diffdf
        elif vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == True:
            diffdf = ODESolver.diffdf_settling
        else:
            diffdf = ODESolver.diffdf_no_mol  # used by Longkang Dai in Venusian middle and lower amtospheres

        diff_term = diffdf(var.y,atm) # note that this is not completely the same as the ODE, and have not dealt with fixed sp and ion
        chem_term = chemdf(var.y,atm.M,var.k) # note that this is not completely the same as the ODE, and have not dealt with fixed sp and ion

        var_save['diff_term'] = diff_term
        var_save['chem_term'] = chem_term'''
        var_save['diff_term'] = var.diff_term
        var_save['chem_term'] = var.chem_term
