...
class Atm(object):
    ...
    def load_TPK(self, data_atm):
        ...
        PTK_fun = {}
        zco = np.loadtxt(vulcan_cfg.alt_path)
        zico = np.zeros(len(zco) - 2)
        for i in range(len(zico)):
            zico[i] = (zco[i + 1] + zco[i + 2]) / 2
        ...
        # for atm_type = 'file' and also Kzz_prof = 'file
            ...
            # Longkang modified for storing Tco directly from PTK file
            data_atm.Tco = T_file
            data_atm.pco = p_file
            PTK_fun['pT'] = interpolate.interp1d(p_file, T_file, kind='slinear')
            '''try:
                data_atm.Tco = PTK_fun['pT'](data_atm.pco)
            ...
            if self.use_Kzz == True and self.Kzz_prof == 'file':
                ...
                PTK_fun['zK'] = interpolate.interp1d(zco[1:],np.log(Kzz_file),kind='slinear')
                data_atm.Kzz = np.exp(PTK_fun['zK'](zico))
        ...
        if self.vz_prof == 'const': data_atm.vz = np.repeat(self.const_vz,nz-1)
            ...
            inter_vz = interpolate.interp1d(zco[1:], atm_table['vz'], kind='slinear')
            data_atm.vz = inter_vz(zico)
        ...
        print('========== Pressure profile (dyne/cm2) =========')
        print(data_atm.pco)
        print('========== Temperature profile (K) =============')
        print(data_atm.Tco)
    ...
    def f_mu_dz(self, data_var, data_atm, output): # Initialising mean molecular weight and dz
        ...
        zco = alt_read # added by Longkang for given altitude
        for i in range(pref_indx,nz):
            ...
            #dz[i] = Hp[i] * np.log(pico[i]/pico[i+1]) # pico[i+1] has a lower P than pico[i] (higer height) # removed by Longkang for given altitude
            #zco[i+1] = zco[i] + dz[i] # zco is set zero at 1bar for gas giants # removed by Longkang for given altitude
            dz[i] = zco[i+1] - zco[i] # added by Longkang for given altitude
        ...
        if not pref_indx == 0:
            for i in range(pref_indx-1,-1,-1):
                ...
                Hp[i] = kb*Tco[i]/(mu[i]/Navo*gz[i])
                #dz[i] = Hp[i] * np.log(pico[i]/pico[i+1]) # removed by Longkang for given altitude
                #zco[i] = zco[i+1] - dz[i] # from i+1 propogating down to i # removed by Longkang for given altitude
                dz[i] = zco[i+1] - zco[i] # added by Longkang for given altitude
    ...
    def read_sflux(self, var, atm):
        ...
        # Longkang modified for using the actinic flux at the top of planetary atmosphere
        if vulcan_cfg.use_sflux_top == True:
            atm.sflux_raw = np.genfromtxt(vulcan_cfg.sflux_top_path, dtype=float, skip_header=1, names=['lambda', 'flux'])
        ...
        if vulcan_cfg.use_sflux_top == True: # Longkang added
            raw_flux = atm.sflux_raw['flux']
        else:
